...
def resume_training(checkpoint_path, start_epoch, additional_epochs, model, optimizer, scheduler, data_loader, device, checkpoint_basename="ssd_checkpoint"):
    ...
    # Save loss curve
    plt.figure()
    plt.plot(range(start_epoch + 1, start_epoch + additional_epochs + 1), loss_history, marker='o')
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Resumed Training Loss Curve")
    plt.grid(True)
    plt.savefig(f"checkpoints/{checkpoint_basename}_resumed_loss_curve.png")
    plt.close()
    print(f"Loss curve saved to checkpoints/{checkpoint_basename}_resumed_loss_curve.png")


if __name__ == "__main__":
    # ====== Modify these before running ======
    checkpoint_path = "checkpoints/ssd_checkpoint_epoch30.pth"  # <- your .pth file
    start_epoch = 30                                            # <- epoch from checkpoint
    more_epochs = 15                                            # <- how many more to train
    checkpoint_basename = "ssd_checkpoint"                      # <- for saving new checkpoints
    voc_root = "../VOCdata"                                     # <- your VOC data path
    # =========================================
    ...
    voc_dataset = get_voc_dataset(root=voc_root, year="2012", image_set="train")
    ...
    final_path = f"checkpoints/final_model_resumed_ssd320.pth"
    torch.save(model.state_dict(), final_path)
    print(f"Final resumed model saved to {final_path}")
